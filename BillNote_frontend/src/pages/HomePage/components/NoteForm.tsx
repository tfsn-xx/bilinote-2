...
const formSchema = z
  .object({
    ...
    grid_size: z
      ...
      .default([2, 2])
      .optional(),
    summary_chunk_count: z.coerce.number().int().min(1, '分段数至少为 1').max(50, '分段数最多为 50'),
    summary_max_concurrency: z.coerce.number().int().min(1, '并发上限至少为 1').max(32, '并发上限最多为 32'),
    summary_retry_count: z.coerce.number().int().min(0, '重试次数不能为负数').max(10, '重试次数最多为 10'),
  })
...
const NoteForm = () => {
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      summary_chunk_count: formData.summary_chunk_count ?? DEFAULT_SUMMARY_CONFIG.summary_chunk_count,
      summary_max_concurrency: formData.summary_max_concurrency ?? DEFAULT_SUMMARY_CONFIG.summary_max_concurrency,
      summary_retry_count: formData.summary_retry_count ?? DEFAULT_SUMMARY_CONFIG.summary_retry_count,
    })
  }, [
    // 当下面任意一个变了，就重新 reset
    currentTaskId,
    ...
    <div className="h-full w-full">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-4">
          ...
          {/* AI 总结配置 */}
          <SectionHeader title="AI 总结配置" tip="生成前可按视频长度和模型限流情况调整" />
          <div className="grid grid-cols-3 gap-3">
            <FormField
              control={form.control}
              name="summary_chunk_count"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>分段数</FormLabel>
