...
export interface TaskProgress {
  ...
  merge_mode?: 'not_applicable' | 'ai_merged' | 'stitched_fallback' | string
...
export interface Task {
  ...
  // 结果文件中的持久化耗时，兼容刷新页面或旧任务没有 progress 的情况。
      ...
      updateTaskContent: (id, data) =>
          set(state => ({
            tasks: state.tasks.map(task => {
              ...
              // 成功轮询可能只是在补齐状态元数据（例如旧任务的总用时），
              // 不能因为 status 没变化就丢弃 progress。
              if (
                task.status === 'SUCCESS' &&
                data.status === 'SUCCESS' &&
                !data.progress &&
                data.generation_duration_seconds === undefined
              ) return task
