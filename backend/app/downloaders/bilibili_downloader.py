...
class BilibiliDownloader(Downloader, ABC):
    def __init__(self):
        ...
        self._proxy_mgr = ProxyConfigManager()
        self.subtitle_failure_reason: Optional[str] = None
        self._subtitle_auth_failed = False

    def _network_options(self) -> dict:
        """Return bounded yt-dlp network retries without logging proxy secrets."""
        try:
            attempts = max(1, int(os.getenv("BILIBILI_RETRY_ATTEMPTS", "3")))
        except (TypeError, ValueError):
            attempts = 3
        try:
            timeout = max(5, int(os.getenv("BILIBILI_SOCKET_TIMEOUT", "20")))
        except (TypeError, ValueError):
            timeout = 20
        options = {
            "retries": attempts,
        ...
        proxy = self._proxy_mgr.get_proxy_url()
        if proxy:
            options["proxy"] = proxy
            logger.info("B站下载使用已配置代理（代理地址已隐藏）")
        return options
    ...
    def download_subtitles(self, video_url: str, output_dir: str = None,
        ...
        """
        # 同一任务前面已经确认 Cookie 无效时，不再重复请求平台或 yt-dlp。
        # 浏览器预取字幕若成功会更早写入缓存，因此这里直接交给 Whisper。
        if self._subtitle_auth_failed:
            self.subtitle_failure_reason = "auth_required"
            logger.info("B站 Cookie 登录态已确认无效，跳过重复字幕请求，自动回退 Whisper")
            return None

        self.subtitle_failure_reason = None
        ...
        try:
            ...
            if result and result.segments:
                return result
            self.subtitle_failure_reason = fetcher.last_failure_reason
            if self.subtitle_failure_reason == "auth_required":
                self._subtitle_auth_failed = True
                logger.warning("B站登录态无效且浏览器预取字幕未命中，自动使用 Whisper 转写")
                return None
        except Exception as e:
            self.subtitle_failure_reason = "subtitle_api_error"
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not subtitles:
                    self.subtitle_failure_reason = self.subtitle_failure_reason or "no_subtitles"
