...
from app.services.note import NoteGenerator, logger, request_task_cancel, reset_task_cancel
...
from app.utils.task_error import TaskErrorCode, TaskGenerationError
...
class VideoRequest(BaseModel):
    ...
    summary_chunk_count: int = Field(default=1, ge=1, le=50)
    summary_max_concurrency: int = Field(default=6, ge=1, le=32)
    summary_retry_count: int = Field(default=2, ge=0, le=10)
...
def _normalize_result_content(result_content: dict, duration_seconds: Optional[float] = None) -> dict:
    """为旧结果补做时间跳转链接和生成耗时，避免历史笔记继续显示旧格式。"""
    if not isinstance(result_content, dict):
        return result_content

    markdown = result_content.get("markdown")
    audio_meta = result_content.get("audio_meta") or {}
    video_id = audio_meta.get("video_id") if isinstance(audio_meta, dict) else None
    platform = audio_meta.get("platform", "bilibili") if isinstance(audio_meta, dict) else "bilibili"
    if isinstance(markdown, str) and video_id:
        result_content["markdown"] = replace_content_markers(markdown, video_id, platform)
    if result_content.get("duration_seconds") is None and duration_seconds is not None:
        result_content["duration_seconds"] = duration_seconds
    return result_content
...
def run_note_task(task_id: str, video_url: str, platform: str, quality: DownloadQuality,
                  ...
                  video_interval=0, grid_size=[], summary_chunk_count: int = 1,
                  summary_max_concurrency: int = 6, summary_retry_count: int = 2
                  ):
    ...
    try:
        ...
        logger.exception("结果文件保存失败 (task_id=%s)", task_id)
...
def generate_note(data: VideoRequest, background_tasks: BackgroundTasks):
    try:
        ...
        background_tasks.add_task(run_note_task, task_id, data.video_url, data.platform, data.quality, data.link,
                                  ...
                                  data.summary_chunk_count, data.summary_max_concurrency, data.summary_retry_count)
...
def get_task_status(task_id: str):
    ...
    if os.path.exists(status_path):
        ...
        if status == TaskStatus.SUCCESS.value:
            ...
            if os.path.exists(result_path):
                ...
                result_content = _normalize_result_content(
                    result_content, status_content.get("duration_seconds")
                )
                return R.success({
                    "status": status,
    ...
    if os.path.exists(result_path):
        ...
        result_content = _normalize_result_content(result_content)
        return R.success({
