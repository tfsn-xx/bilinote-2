...
def _collect_chat_stream(stream: object):
    """Consume a streamed chat response while preserving tool calls.

    The HTTP API of this service still returns one JSON answer after the RAG
    and tool loop completes, but every upstream model request is made in
    streaming mode.  Aggregating here lets tool calling continue to use the
    existing synchronous-looking control flow without sending a ``Sync``
    request to an OpenAI-compatible relay.
    """
    # Compatibility for a test double or an older SDK that ignores stream=True.
    ...
    message = SimpleNamespace(
        ...
        tool_calls=aggregated_calls or None,
    )
    return SimpleNamespace(choices=[SimpleNamespace(message=message)])


def _assistant_message_payload(message) -> dict:
    """Convert an aggregated assistant message into the next request shape."""
    payload = {"role": "assistant", "content": message.content or None}
...
def chat(
    ...
    for round_i in range(max_rounds):
        ...
        response = _collect_chat_stream(stream)
